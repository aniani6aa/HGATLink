...
class GCMCGraphConv(nn.Module):
    def __init__(
        ...
        if relation_att:
            self.relation_att = nn.Parameter(th.Tensor(heads, d_k, d_k))
        else:
            self.register_parameter("relation_att", None)
    ...
    def forward(self, graph, feat, weight=None,weight_k=None,relation_att=None):
        ...
        with graph.local_scope():
            ...
            if weight is not None:
                ...
                relation_att=dot_or_identity(res, relation_att, self.device)
            #Attention mechanisms
            q_mat = (weight_k * ci).view(-1, self.n_heads, self.d_k)
            
            k_mat = q_mat
            q_norm_l = F.normalize(q_mat, p=2., dim=-1)
            k_norm_r = F.normalize(k_mat, p=2., dim=-1)
            alpha = torch.abs(k_norm_r * q_norm_l).view(k_norm_r.size(0), -1)
            Tau = 0.25
